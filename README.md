[![logo](https://raw.githubusercontent.com/dgspai/openvpn-client/master/logo.png)](https://openvpn.net/)
...
[file](https://github.com/dgspai/openvpn-client/raw/master/docker-compose.yml).
...
## Starting an OpenVPN client instance
    ...
    sudo docker run -it --cap-add=NET_ADMIN --device /dev/net/tun --name vpn \
                -v /some/path:/vpn -d dgspai/openvpn-client \
...
you'll also need to run a web proxy, like so:
    ...
    sudo docker run -it --name web -p 80:80 -p 443:443 \
                --link vpn:<service_name> -d dgspai/nginx \
...
a concrete example:
    ...
    sudo docker run -it --name bit --net=container:vpn -d dgspai/transmission
    sudo docker run -it --name web -p 80:80 -p 443:443 --link vpn:bit \
                -d dgspai/nginx -w "http://bit:9091/transmission;/transmission"

For multiple services (non-existant 'foo' used as an example):

    sudo docker run -it --name bit --net=container:vpn -d dgspai/transmission
    sudo docker run -it --name foo --net=container:vpn -d dgspai/foo
    sudo docker run -it --name web -p 80:80 -p 443:443 --link vpn:bit \
                --link vpn:foo -d dgspai/nginx \
...
## Configuration
    ...
    sudo docker run -it --rm dgspai/openvpn-client -h
...
### Setting the Timezone
    ...
    sudo cp /path/to/vpn.crt /some/path/vpn-ca.crt
    sudo docker run -it --cap-add=NET_ADMIN --device /dev/net/tun --name vpn \
                -v /some/path:/vpn -e TZ=EST5EDT -d dgspai/openvpn \
                -v 'vpn.server.name;username;password'
...
Or you can store it in the container:
    ...
    cat /path/to/vpn.crt | sudo docker run -it --cap-add=NET_ADMIN \
                --device /dev/net/tun --name vpn -d dgspai/openvpn-client \
...
block all outbound traffic if the VPN is down.
    ...
    sudo cp /path/to/vpn.crt /some/path/vpn-ca.crt
    sudo docker run -it --cap-add=NET_ADMIN --device /dev/net/tun --name vpn \
                -v /some/path:/vpn -d dgspai/openvpn-client -f "" \
                -v 'vpn.server.name;username;password'
...
`docker run` statement. Here's an example of doing so, with a Google DNS server:
    ...
    sudo cp /path/to/vpn.crt /some/path/vpn-ca.crt
    sudo docker run -it --cap-add=NET_ADMIN --device /dev/net/tun --name vpn \
                --dns 8.8.4.4 -v /some/path:/vpn -d dgspai/openvpn-client \
                -v 'vpn.server.name;username;password'
...
but adding your vpn user and password by command line
    ...
    sudo docker run -it --cap-add=NET_ADMIN --device /dev/net/tun --name vpn \
            -v /some/path:/vpn -d dgspai/openvpn-client -a 'username;password'
...
through a [GitHub issue](https://github.com/dgspai/openvpn-client/issues).